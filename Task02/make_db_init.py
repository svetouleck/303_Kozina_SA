...
with open("db_init.sql", "w", encoding="utf-8") as fout:
    print(".open movies_rating.db", end="\n", file=fout)
    ...
    with open("..\dataset\movies.csv", "r", encoding="utf-8") as f:
        ...
        for i in range(len(file)):
            ...
            if i != len(file)-1:
                print(f"({item[0]}, '{item[3]}', {item[2]}, '{item[1]}')", end=",\n", file=fout)
            else:
                print(f"({item[0]}, '{item[3]}', {item[2]}, '{item[1]}')", end=";\n", file=fout) 
    ...
    with open("../dataset/ratings.csv", "r") as fl:
        ...
        for i in range(len(file)):
            item = file[i][:-1].split(",")
            if i != len(file)-1:
                print(f"({i}, {item[0]}, {item[1]}, {item[2]}, {item[3]})", end=",\n", file=fout)
            else:
                print(f"({i}, {item[0]}, {item[1]}, {item[2]}, {item[3]})", end=";\n", file=fout) 
    ...
    with open("../dataset/users.txt", "r") as fl:
        ...
        for i in range(len(file)):
            ...
            item = item.split("|")
            if i != len(file)-1:
                print(f"( {item[0]}, '{item[1]}', '{item[2]}', '{item[3]}', {item[4]}, '{item[5]}')", end=",\n", file=fout)
            else:
                print(f"( {item[0]}, '{item[1]}', '{item[2]}', '{item[3]}', {item[4]}, '{item[5]}')", end=";\n", file=fout) 
